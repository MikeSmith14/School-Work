...
const byte DIRA = 12;
const byte DIRB = 13;
const int trigPin = 9;    //Sonar Pins
const int echoPin = 10;   //Sonar Pins
...
int distance; //Sonar Bounce Back Variable

//Line Check threshold
int threshold = 800;

NewPing sonar(trigPin, echoPin, maxDistance); //Sonar Declaration
...
void loop() {
  distance = sonar.ping_median();     //Defining distance
  ...
  tcs.getRawData(&r, &g, &b, &c);
   if (c > 50){      //need to recalibrate depending on surface and lighting
    ...
    if(distance < 12 && distance !=4 && distance != 0 && distance != 5){    //If sonar's distance it will pivot away from object
      pivot();
    }
    if(distance <10){
      pivot();
...
//Attempts to find line after avoiding object
void searchLine() {
  forward();
  delay(500);
...
//Pivot away from the object and search for line
void pivot(){
    brake();
